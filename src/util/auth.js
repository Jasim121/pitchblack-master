...
import { useUser, createUser, updateUser, db } from "./db";
...
const initData = async (userId, userEmail) => {
  const userName = userEmail.match(/^([^@]*)@/)[1];

  const companyId = nanoid(6);
  await setDoc(doc(db, "Founder", userId), {
    id: userId,
    name: userName,
    email: userEmail,
    companies: [
...
function useAuthProvider() {
  ...
  const handleAuth = async (response) => {
    ...
    if (isNewUser) {
      ...
      await initData(user.uid, user.email);
